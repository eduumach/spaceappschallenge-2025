...
export class NASATemporalService {
  ...
  static async fetchTemporalData(request: NASATemporalRequest): Promise<NASATemporalResponse> {
    // Validar coordenadas
    ...
    // Converter datas string para Date objects para validação
    const startDateObj = new Date(
      parseInt(request.startDate.substring(0, 4)),
      parseInt(request.startDate.substring(4, 6)) - 1,
      parseInt(request.startDate.substring(6, 8))
    )
    
    const endDateObj = new Date(
      parseInt(request.endDate.substring(0, 4)),
      parseInt(request.endDate.substring(4, 6)) - 1,
      parseInt(request.endDate.substring(6, 8))
    )
    ...
    // Parâmetros padrão se não especificados
    ...
    url.searchParams.set('community', request.community || 'AG')
    url.searchParams.set('format', request.format || 'JSON')
    url.searchParams.set('header', (request.header !== false).toString())
    url.searchParams.set('time-standard', request.time_standard || 'LST')
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 400) {
          const errorData: NASAErrorResponse = await response.json()
          const errorMessage = errorData.messages?.[0]?.text || 'Parâmetros inválidos'
      ...
      if (error instanceof Error) {
        ...
        throw new Error('Erro desconhecido ao consultar dados da NASA')
      }
    }
  }

  // Método de conveniência para buscar dados com range de datas
  static async fetchDataForDateRange(
    latitude: number,
    longitude: number,
    startDate: Date,
    endDate: Date,
    parameters?: string[]
  ): Promise<NASATemporalResponse> {
    return this.fetchTemporalData({
      latitude,
      longitude,
